...
int main(void) {
    ...
    /* A warm 16 GiB mapping may leave almost no free pages while retaining a
     * large inactive file cache.  Count the second half of those pages only
     * far enough to restore the measured 259-expert floor; do not let warmth
     * buy a larger tier. */
    memory = (ds4_ssd_host_memory){
        .physical_bytes = 16 * GIB,
        .recommended_bytes = 12 * GIB,
        .free_bytes = 64 * MIB,
        .purgeable_bytes = 192 * MIB,
        .inactive_bytes = 7 * GIB,
        .file_backed_bytes = 8 * GIB,
        .memory_pressure = DS4_SSD_MEMORY_PRESSURE_NORMAL,
    };
    assert(ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        flash_expert_bytes, flash_max_cacheable, &adaptive));
    assert(adaptive.reclaimable_bytes == 15 * GIB / 4u);
    assert(adaptive.low_ram_floor_bridge_bytes ==
           adaptive.floor.minimum_cache_bytes - GIB);
    assert(adaptive.current_wire_budget_bytes ==
           adaptive.floor.minimum_cache_bytes);
    assert(adaptive.wire_budget_bytes == adaptive.floor.minimum_cache_bytes);
    assert(adaptive.cache_experts == 259);

    /* Inactive cache is not a blanket escape hatch: if even its full amount
     * cannot preserve reserves plus the floor, AUTO still refuses. */
    memory = (ds4_ssd_host_memory){
        .physical_bytes = 16 * GIB,
        .recommended_bytes = 12 * GIB,
        .free_bytes = GIB / 4u,
        .inactive_bytes = 4 * GIB,
        .file_backed_bytes = 8 * GIB,
        .memory_pressure = DS4_SSD_MEMORY_PRESSURE_NORMAL,
    };
    assert(!ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        flash_expert_bytes, flash_max_cacheable, &adaptive));
    assert(adaptive.low_ram_floor_bridge_bytes == 0);

    const uint64_t low_ram_reserve = 11 * GIB / 4u;
    const uint64_t flash_floor_bytes =
        UINT64_C(259) * flash_expert_bytes;
    memory = (ds4_ssd_host_memory){
        .physical_bytes = 16 * GIB,
        .recommended_bytes = 12 * GIB,
        .inactive_bytes = low_ram_reserve + flash_floor_bytes,
        .file_backed_bytes = low_ram_reserve + flash_floor_bytes,
        .memory_pressure = DS4_SSD_MEMORY_PRESSURE_NORMAL,
    };
    assert(ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        flash_expert_bytes, flash_max_cacheable, &adaptive));
    assert(adaptive.low_ram_floor_bridge_bytes == flash_floor_bytes);
    assert(adaptive.cache_experts == 259);

    memory.memory_pressure = DS4_SSD_MEMORY_PRESSURE_WARNING;
    assert(!ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        flash_expert_bytes, flash_max_cacheable, &adaptive));
    assert(adaptive.low_ram_floor_bridge_bytes == 0);
    ...
    /* A similarly sized but unmeasured model geometry cannot use the bridge. */
    assert(!ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 2, 1,
        512 * MIB, 1024, &adaptive));
    assert(adaptive.floor.minimum_cache_experts == 3);
    assert(adaptive.low_ram_floor_bridge_bytes == 0);

    /* The bridge boundary is exact and uses the smaller of inactive and
     * file-backed pages. */
    ...
    assert(adaptive.low_ram_floor_bridge_bytes == 0);
    memory.inactive_bytes++;
    memory.file_backed_bytes--;
    assert(!ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        flash_expert_bytes, flash_max_cacheable, &adaptive));
    assert(adaptive.low_ram_floor_bridge_bytes == 0);

    /* One byte above the low-RAM host boundary keeps the bridge disabled. */
    memory.physical_bytes = 16 * GIB + 1u;
    memory.file_backed_bytes++;
    assert(!ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        flash_expert_bytes, flash_max_cacheable, &adaptive));
    ...
    /* Even the 259-entry geometry stays fail-closed if its floor exceeds the
     * absolute 2 GiB validated bridge cap. */
    const uint64_t oversized_expert_bytes = 9 * MIB;
    const uint64_t oversized_floor_bytes =
        UINT64_C(259) * oversized_expert_bytes;
    memory = (ds4_ssd_host_memory){
        .physical_bytes = 16 * GIB,
        .recommended_bytes = 12 * GIB,
        .inactive_bytes = low_ram_reserve + oversized_floor_bytes,
        .file_backed_bytes = low_ram_reserve + oversized_floor_bytes,
        .memory_pressure = DS4_SSD_MEMORY_PRESSURE_NORMAL,
    };
    assert(!ds4_ssd_adaptive_cache_plan_make_with_static_reserve(
        &memory, 512 * MIB, 8 * GIB, false, 43, 6,
        oversized_expert_bytes, flash_max_cacheable, &adaptive));
    assert(adaptive.floor.minimum_cache_experts == 259);
    assert(adaptive.floor.minimum_cache_bytes > 2 * GIB);
    assert(adaptive.low_ram_floor_bridge_bytes == 0);
