...
struct ds4_metal_args_qwen35_router_refine {
    uint32_t n_token;
    uint32_t reserved;
    uint64_t logits_token_stride;
    uint64_t input_token_stride;
...
// Select a conservative approximate top-32 before exact F32 refinement.  The
// compensated tiled router supplies the initial ordering; one threadgroup per
// token keeps candidate selection deterministic and lower-ID stable on ties.
kernel void kernel_qwen35_router_top32_candidates_f32(
        constant ds4_metal_args_qwen35_router_refine &args [[buffer(0)]],
        device const char *logits     [[buffer(1)]],
        device       char *candidates [[buffer(2)]],
        uint3 group [[threadgroup_position_in_grid]],
        ushort tid [[thread_index_in_threadgroup]]) {
    constexpr uint n_expert = 256u;
    constexpr uint n_candidate = 32u;
    const uint token = group.x;
    if (tid != 0u || token >= args.n_token) return;

    const uint64_t logits_base =
        (uint64_t)token * args.logits_token_stride;
    const uint64_t candidate_base =
        (uint64_t)token * args.candidate_token_stride;
    int32_t chosen[n_candidate];
    for (uint slot = 0u; slot < n_candidate; slot++) {
        uint best = n_expert;
        for (uint expert = 0u; expert < n_expert; expert++) {
            ...
            const float value = *((device const float *)(
                logits + logits_base + (uint64_t)expert * sizeof(float)));
            const float best_value = best < n_expert
                ? *((device const float *)(
                    logits + logits_base + (uint64_t)best * sizeof(float)))
                : -INFINITY;
            if (best == n_expert || value > best_value ||
                (value == best_value && expert < best)) {
                best = expert;
            }
        }
        chosen[slot] = (int32_t)best;
        *((device int32_t *)(candidates + candidate_base +
            (uint64_t)slot * args.candidate_stride)) = (int32_t)best;
    }
}

// Re-evaluate two approximate candidates per threadgroup with the same F32
// vector loads, dot-product order, eight SIMD groups, and two-stage reduction
// as kernel_mul_mv_f32_f32_4.  Exact candidate logits are written in place;
// the normal stable top-8 kernel then consumes the refined vector.
kernel void kernel_qwen35_router_refine_top32_f32(
        constant ds4_metal_args_qwen35_router_refine &args [[buffer(0)]],
        device const char *weights    [[buffer(1)]],
        device const char *input      [[buffer(2)]],
        device       char *logits     [[buffer(3)]],
        device const char *candidates [[buffer(4)]],
        threadgroup float *scratch [[threadgroup(0)]],
        uint3 group [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]],
        ushort simdgroup [[simdgroup_index_in_threadgroup]],
        ushort simd_width [[threads_per_simdgroup]]) {
    constexpr uint n_pair = 16u;
    constexpr uint n_row = 2u;
    constexpr uint n_simdgroup = 8u;
    constexpr uint in_dim = 2048u;
    constexpr uint nb = in_dim/32u;
    constexpr uint nf = 16u;
    constexpr uint nf4 = nf/4u;
    const uint pair = group.x;
    const uint token = group.y;
    if (pair >= n_pair || token >= args.n_token ||
        simd_width != 32u || simdgroup >= n_simdgroup) {
        return;
    }

    const uint64_t candidate_base =
        (uint64_t)token * args.candidate_token_stride +
        (uint64_t)(pair*n_row) * args.candidate_stride;
    int32_t expert[n_row];
    device const float4 *weight4[n_row];
    for (uint row = 0u; row < n_row; row++) {
        expert[row] = *((device const int32_t *)(
            candidates + candidate_base +
            (uint64_t)row * args.candidate_stride));
        weight4[row] = (device const float4 *)(
            weights + (uint64_t)expert[row] * args.weight_row_stride);
    }
    device const float4 *input4 = (device const float4 *)(
        input + (uint64_t)token * args.input_token_stride);

    float sum[n_row] = {0.0f, 0.0f};
    const uint ix = lane/2u;
    const uint il = lane%2u;
    const uint ib0 = simdgroup*nf + ix;
    device const float4 *input_block =
        input4 + (ib0*32u + il*nf)/4u;

    for (uint ib = ib0; ib < nb; ib += n_simdgroup*nf) {
        float4 activation[nf4];
        FOR_UNROLL (uint i = 0u; i < nf4; i++) {
            activation[i] = input_block[i];
        }
        FOR_UNROLL (uint row = 0u; row < n_row; row++) {
            device const float4 *weight_block =
                weight4[row] + (ib*32u + il*nf)/4u;
            float partial = 0.0f;
            FOR_UNROLL (uint i = 0u; i < nf4; i++) {
                partial += dot(weight_block[i], activation[i]);
            }
            sum[row] += partial;
        }
        input_block += n_simdgroup*nf*32u/4u;
    }

    FOR_UNROLL (uint row = 0u; row < n_row; row++) {
        threadgroup float *row_scratch = scratch + row*32u;
        if (simdgroup == 0u) row_scratch[lane] = 0.0f;
        sum[row] = simd_sum(sum[row]);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    FOR_UNROLL (uint row = 0u; row < n_row; row++) {
        if (lane == 0u) scratch[row*32u + simdgroup] = sum[row];
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    FOR_UNROLL (uint row = 0u; row < n_row; row++) {
        const float total = simd_sum(scratch[row*32u + lane]);
        if (lane == 0u && simdgroup == 0u) {
            *((device float *)(logits +
                (uint64_t)token * args.logits_token_stride +
                (uint64_t)expert[row] * sizeof(float))) = total;
        }
    }
}
