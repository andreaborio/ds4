...
| Qwen3.6-35B-A3B ExpertMajor v2 (`qwen35moe`, dual weight codecs) | one model/session/graph runtime with exact MLX Affine4 G64 and Q2_K_XL profile binding; shared GDN, attention, KV, routing, resident/SSD admission, cache, and scheduling; codec-specific physical weight kernels only | **FORK MAIN RUNTIME; AFFINE4 STABLE AND Q2_K_XL BETA PUBLISHED; UPSTREAM ISSUE [#462](https://github.com/antirez/ds4/issues/462)** — the Stable `Qwen3.6-35B-A3B-Hebrus-ExpertMajor-v2-MLX-Affine4-G64.gguf` is 20,808,566,880 bytes with SHA-256 `dd17266185833a9f05531ce366fd7284ddca1ed64aa3dcf06e321e8c72c9ea3d`. The opt-in `Qwen3.6-35B-A3B-Hebrus-ExpertMajor-v2-Q2_K_XL.gguf` is 12,290,632,032 bytes with SHA-256 `30c22f70aff0f05986b517ee4ad8fef554a1b5aab6971c9ca09f999566d30143`. Both canonical paths are pinned at repository revision `e002665becd2db618897effb213030fdf92e7e98`; the Beta is available only through its separate `qwen-q2-beta` selector and remains nonrecommended pending endpoint qualification. Affine2 was rejected and removed rather than retained as a third codec. The former Q4_K_S v2 artifact remains negative-only; v1, sidecar, canonical, and non-Metal inference paths remain retired. |
| Qwen3.8-Flash-Next / Qwen4Exp text-only bring-up (2026-08-30 Phase 0-4) | freeze the exact checkpoint/source inventory and model math, add model-free scalar/tokenizer/chat oracles, close the ExpertMajor family-4 and PLE v1 structures, then prove exact metadata/inventory/store/page ownership through a sparse admission fixture before execution work | **FORK RESEARCH, PINNED-NOT-SUPPORTED** — Normal builds intentionally register no Qwen4Exp physical profile. A separate CPU-only test-hook binary admits one BF16 structural fixture for inspection, reports runtime/payload support false, and rejects generation before runtime setup. Phase 4 closes the tokenizer.json-authoritative NFC/BPE contract, the 248,077 effective-token versus 248,320 physical-logit boundary, all 33 added tokens, the exact stop set and a segmented chat renderer that prevents client literals becoming trusted controls. It does not enable a runtime or server route. MLX affine G64 remains only a non-release ExpertMajor structural candidate, the PLE codec remains unselected, and runtime, Metal, SSD execution, server, downloader and product support stay disabled until every later correctness, memory, performance, artifact and product gate passes. |
| Qwen hardware-aware Metal policy (2026-07-21; 24 GiB guard amended 2026-07-24 and 2026-07-27) | named 16/24/32/36/48/64/96/128 GiB profiles, deterministic SSD-only admission through 24 GiB, continuous larger-host resident reserve, normal-pressure cold/warm file-cache equivalence, and per-slab live admission | **FORK MAIN BASE; 2026-07-27 HARDENING IS A LOCAL RELEASE CANDIDATE; 24 GiB PHYSICAL RECHECK REQUIRED** — AUTO resolves to SSD on M1 Pro 16 GiB and resident on AC-powered M1 Pro 32 GiB with zero swap. Split-K transfer is independently confirmed at 2K on 16 GiB (15.00 versus 9.70 decode t/s) and 32K on 32 GiB (19.02 versus 1.63 decode t/s), with identical greedy token IDs in each A/B. A reported M5 24 GiB sustained decode reached macOS `WARNING` under the formerly uncapped cache. The hardened candidate rejects resident through 24 GiB, keeps the 3,521-expert target, rechecks every proposed slab (up to 321 experts; the final target tail may be smaller), and freezes at already allocated capacity with eviction/reuse on denial. Synthetic 21 GiB and real Metal fault-injection regressions pass, but the versioned five-request physical gate in Hebrus Studio must pass before the incident is signed off. |
