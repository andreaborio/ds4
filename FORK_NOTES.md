...
| Qwen3.8-Flash-Next / Qwen4Exp text-only bring-up (2026-08-30 Phase 0-2) | freeze the exact checkpoint/source inventory and model math, add a model-free scalar oracle, then close the ExpertMajor family-4 geometry, PLE v1 structural format and complete header/index-only conversion map before loader or execution work | **FORK RESEARCH, PINNED-NOT-SUPPORTED** — Phase 2 retains ExpertMajor v2, raises only its structural expert limit to 512, and uses MLX affine G64 solely as a non-release structural candidate because 640-wide rows reject 256-element K-quant blocks. The PLE codec remains unselected. Runtime, Metal, SSD execution, server, downloader and product support remain disabled until every later correctness, memory, performance, artifact and product gate passes. |
