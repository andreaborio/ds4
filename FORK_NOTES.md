...
| Qwen3.8-Flash-Next / Qwen4Exp text-only bring-up (2026-08-30 Phase 0-1) | freeze the exact checkpoint/source inventory and model math, then add a model-free immutable profile plus independent scalar C oracle with pinned upstream captures before any converter, loader, Metal, SSD, server, or product exposure | **FORK RESEARCH, PINNED-NOT-SUPPORTED** — the source revision, Transformers commit, 128-shard inventory, license, tensor roles, layer pattern, PLE extent/hash constants, byte formulas, and scalar semantics are evidence only. Runtime support remains disabled until every later phase and the normal correctness, memory, performance, artifact, and product gates pass. No support claim or upstream proposal is implied by the Phase 0-1 files. |
