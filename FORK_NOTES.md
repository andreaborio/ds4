...
| Qwen3.8-Flash-Next / Qwen4Exp text-only bring-up (2026-08-30 Phase 0-5) | freeze the exact checkpoint/source inventory and model math; close artifact, admission, tokenizer and chat contracts; then port the reviewed llama.cpp graph/state order into transactional model-free host and Metal fixtures backed by pinned Transformers oracles | **FORK RESEARCH, PINNED-NOT-SUPPORTED** — Normal builds intentionally register no Qwen4Exp physical profile. Phase 5 links an unselected resident coordinator and inert Metal correctness partition, but product execution still fails at physical-profile admission. Token/chunk/multi-turn state, PLE ubatch convolution, QSA 2051/2052 and 262K rejection, Metal OOM and command/status rollback are qualified. MLX affine G64 remains only a non-release ExpertMajor structural candidate, the PLE codec remains unselected, and production execution, sparse QSA, SSD execution, server, downloader and product support stay disabled until the later correctness, memory, performance, artifact and product gates pass. |
