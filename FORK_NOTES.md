...
upstream `main` `6d5572d`, audited 2026-07-20.
...
| Qwen3.6-35B-A3B ExpertMajor v2 (`qwen35moe`, 2026-07-20 release gate) | distinct v2 family ID, generic converter, embedded single-layout Q4_K_S store, AUTO resident/SSD admission, model-backed output, and existing resident prefill/parallel decode schedules | **FORK MAIN; UPSTREAM ISSUE [#462](https://github.com/antirez/ds4/issues/462)** — the experimental guard, v1 loader, sidecar, and canonical inference paths are retired. The 20,808,566,880-byte v2 artifact has SHA-256 `d7c43a6388ec20e6fe5530850350f96fdb0ac37c5ce36d3e5f92b172c447f56b`. Resident v2/v1/v2 at 2K measured 318.96/29.54, 320.59/29.59, and 318.83/29.54 prefill/decode t/s; evidence was byte-identical and no new swapout occurred. The v1 arm is historical control evidence only. |
| Exact paired Q8 Metal decode (2026-07-20 long-context candidate) | compute two independent Q8 matvec outputs in one exact Metal dispatch while preserving bit-identical accumulation order | **FORK CANDIDATE; M5 METAL ONLY** — Qwen 32K A/B/B/A kept throughput neutral (+0.057%) and reduced mean p95 from 337.329 to 334.230 ms (-0.92%), beyond both control and candidate spread. All arms had identical evidence and zero swap. Other Apple families retain the exact scalar dispatch. Do not advertise a general t/s gain. |
| DeepSeek phase-adaptive long-context cache + SIMD router (2026-07-20 candidate) | shrink the ExpertMajor cache only during large prefill, restore 4,129 records through 32K or 2,065 at 65K+, pressure-gate growth, seed once after success, and fuse exact one-token top-6 finalize/weights work | **FORK CANDIDATE; M5 64 GB FINAL RERUN OPEN** — the preceding candidate measured 161.68/8.46 and 137.04/7.46 prefill/decode t/s at 8K/32K. Isolated AUTO-to-SSD 65K/100K measured 141.61/5.72 and 126.72/5.58 with zero swap; the rejected 4,129-record 65K arm swapped before decode. Non-M5 Apple systems retain the exact generic cache/router path. No standalone SIMD percentage is claimed until a valid isolated cohort exists. |
| GLM compact-indexer SSD transition (2026-07-20 candidate) | map the 21 exact norm/bias view pairs required after final prefill output mapping and invalidate static-map state before remap | **FORK CORRECTNESS CANDIDATE** — fixes the 32K failure at the full-attention/indexed frontier without changing KV or attention geometry. Final 32K+128 measured 36.19 prefill t/s and 1.79 t/s p50 steady decode, with zero swapout and exact logits/evidence. Baseline emits no logits, so this is not a speedup claim. |
